 ...
 * @Version V1.0
 */
public class FullImageActivity  extends Activity implements ImagePlayer.ImagePlayerListener,View.OnClickListener, View.OnFocusChangeListener{
    ...
    private ImageButton mPlayBtn,mRotateL,mRotateR;
    private int mDegress;
    private ArrayList<Uri> mImageList = new ArrayList<Uri>();
    private LinearLayout mPlayLay,mRotateLLay,mRotateRlay;
    @Override   
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(DEBUG) Log.d(TAG,"uri:"+uri+" contentType:"+contentType);
        if (uri == null) {
            findPicList();
        } else {
            mCurPicPath = getPathByUri(uri);
            if(DEBUG) Log.d(TAG,"contentType:"+contentType);
        }
    }
        ...
        public void handleMessage(Message msg) {
            switch(msg.what){
                ...
                case DISPLAY_SHOW:
                    ...
                    if(DEBUG) Log.d(TAG,"handle Message mImageList size:"+mImageList.size()+" mSlideIndex:"+mSlideIndex);
    ...
    private String filterPath(String filePath){
        if(filePath.startsWith("/storage/emulated/0")){
            return Environment.getLegacyExternalStorageDirectory().getAbsolutePath()
    ...
    private void findPathByUri(Uri uri, String[] type) {
        ...
        if(DEBUG) Log.d(TAG,"findPathByUri uri is "+uri+" type is "+type);
        if (uri == null) return;
        mSlideIndex = 0;
        String[] projection = {MediaStore.Images.Media.DATA};
        String selection = MediaStore.Images.Media.MIME_TYPE+"=?";
        Cursor mCursor = this.managedQuery(uri, projection, selection, type, MediaStore.Images.Media.DATE_MODIFIED+" desc");
        mImageList.clear();
        if(mCursor!=null){
            mCursor.moveToFirst();
    ...
    private String getContentType(Intent intent) throws RuntimeException {
        ...
        try {
            ...
            if(DEBUG) Log.w(TAG, "get type fail", t);
            return null;
        }
    }
    ...
    private void displayMenu(boolean show){
        if (!show) {  
            ...
            if(DEBUG) Log.d(TAG,"displayMenu set menu visible");
    ...
    protected void onResume() {
        ...
        mSurfaceView.getHolder().setFormat(PixelFormat.VIDEO_HOLE);
        mSurfaceView.getHolder().setFormat(PixelFormat.VIDEO_HOLE_REAL);
    ...
    protected void onPause() {
        ...
        if(null != mImageplayer){
            if(DEBUG) Log.d(TAG,"onPause release");
    ...
    @Override
    public void onShow() {
        if(DEBUG) Log.d(TAG,"onShow");
        mDegress  = 0;
        mUIHandler.sendEmptyMessageDelayed(DISMISS_PROGRESSBAR,1000);
        if(mPlayPicture && mImageList.size()>1){
            if(DEBUG) Log.d(TAG,"onShow:"+mSlideIndex+"mImageList.size():"+mImageList.size());
    ...
    public void onClick(View v) {
        switch(v.getId()){
            ...
            case R.id.menu_left_rotate:
                if(mImageplayer!=null&&mImageplayer.getRunningStatus()==ImagePlayer.STATUS_PLAYING){
                    mDegress-=90;
